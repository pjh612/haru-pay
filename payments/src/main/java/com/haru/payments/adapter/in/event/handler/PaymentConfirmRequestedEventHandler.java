...
public class PaymentConfirmRequestedEventHandler {
    private static final String CONSUMER_KEY = "payments.payment-confirm-requested";
    private final ApplicationEventPublisher eventPublisher;
    private final AlertManager alertManager;
    private final ProcessedEventLogRepository processedEventLogRepository;

    @Transactional
    public void handle(UUID eventId, PaymentConfirmRequestedEventPayload payload) {
        if (!processedEventLogRepository.markIfFirst(CONSUMER_KEY, eventId)) {
            log.info("Duplicate payment-confirm-requested event skipped. eventId={}", eventId);
            return;
        }
